...
int sacarActualLista(tLista* lista,void* dato,unsigned tam){
    ...
    memcpy(dato,actual->info,MINLISTA(actual->tam,tam));
    free(actual->info);
    free(actual);
    if(auxSig){
        auxSig->ant=auxAnt;
...
int sacarPorClaveOrdenadoLista(tLista* lista,void* dato,unsigned tam,Cmp cmp){
    ...
    memcpy(dato,actual->info,MINLISTA(tam,actual->tam));
    free(actual->info);
    free(actual);
    return TODO_OK;
}
...
//
//tNodoLista** buscarMenor(tLista* lista,Cmp cmp){
//    tNodoLista** menor=lista;
//    while((*lista)->sig){
//        if(cmp((*menor)->info,(*lista)->sig->info)>0){
//            menor=&(*lista)->sig;
//        }
//        lista=&(*lista)->sig;
//    }
